...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            email_body = """
Welcome to the cBay.  Please click the link below to
...
""" % (request.get_host(),
       ...
       # login(request, new_user)
            return render(request, 'cbayweb/needsConfirmation.html', {'email':user.email})
        else:
            print(form.errors)
...
def viewProfile(request, user_id):
    ...
    is_following = False
    if my_profile.followings.filter(id = user.id):
        is_following = True
    return render(request, 'cbayweb/viewProfile.html', {'profile': profile, 'is_following':is_following})


@login_required
def messagePicture(request, message_id):
    message = get_object_or_404(Message, id=message_id)
    if not message.message_pic:
        raise Http404
    content_type = guess_type(message.message_pic.name)
    return HttpResponse(message.message_pic, content_type=content_type)



@login_required
def addToFollowing(request):
    user_id = request.GET.get('user_id')
    if user_id:
        user = get_object_or_404(User, id = user_id)
        profile = get_object_or_404(Profile, user = request.user)
        if profile.followings.filter(id = user.id).exists():
            pass
        else:
            profile.followings.add(user)
            profile.save()
            print('Add To Following')
    else:
        pass
    return redirect('/')
...
def confirmRegistration(request, username, token):
    ...
    user.is_active = True
    user.save()
    return render(request, 'cbayweb/confirmed.html', {})
